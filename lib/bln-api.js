...
var UploadWatcher = require('./upload-watcher.js');

var BlnApiError = require('../errors/bln-api.js');
var BlnApiProgress = require('./bln-api-progress.js');
...
var blnApi = {
  queryNodes: function(query, callback) {
    ...
    function doRequest() {
      ...
      blnApiRequest.sendRequest('get', '/v2/nodes', params, (err, result) => {
  ...
  getChildren: function(id, params, callback) {
    ...
    blnApiRequest.sendRequest('get', '/collection/children', params, (err, result) => {
      if(err) return callback(err);
  ...
  whoami: function(callback) {
    blnApiRequest.sendRequest('get', '/v2/users/whoami', (err, result) => {
      if(err) return callback(err);
      callback(null, result);
    })
  },

  getNodeAttributeSummary: function(params, callback) {
    blnApiRequest.sendRequest('get', '/user/node-attribute-summary', params, (err, result) => {
      if(err) return callback(err);
  ...
  createCollection: function(node, callback) {
    ...
    blnApiRequest.sendRequest('post', '/collection', {p: nodePath}, (err, result) => {
      if(err) return callback(err);
  ...
  uploadFile: function(node, callback) {
    ...
    if(stat.size > BigInt(Number.MAX_SAFE_INTEGER)) {
      return callback(new BlnApiError('File size is > MAX_SAFE_INTEGER', 'E_BLN_API_OUT_OF_RANGE'));
    }

    var bytesTotal = parseInt(stat.size);
    var bytesSent = 0;
    ...
    var watcher = new UploadWatcher(nodePath);
    ...
    transferDb.getUploadByTransferId(transferId, (err, foundTransfer) => {
      ...
      watcher.once('changed', () => {
        var err = new BlnApiError('File to upload has changed or was deleted', 'E_BLN_API_UPLOAD_FILE_CHANGED');
        currentReq.abort();
        endRequest(err);
      });
    ...
    function startRequest() {
      async.whilst(
        ...
        (cb) => {
          ...
          var start = chunksComplete * chunkSize;
          var end = (chunksComplete + 1) * chunkSize - 1;
  ...
  downloadFile: function(remoteId, version, node, callback) {
    ...
    function startRequest(offset) {
      ...
      req.on('bln-error', function(err, response) {
        ...
        if(response) {
          ...
          req.on('end', () => {
            ...
            logger.info('Got response', {body: body, status: response.statusCode});
          });
  ...
  nodeDelta: function(params, callback) {
    var requestParams = {attributes: ['parent', 'hash', 'version', 'size']};
