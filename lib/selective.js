...
var knownSharesDb = require('./known-shares-db.js');
...
var selective = {
  ...
  updateIgnoredNodes: function(difference, callback) {
    ...
    this.updateRemotePaths(err => {
      ...
      if(!rootNode || !rootNode.children || rootNode.children.length === 0) {
        ...
        logger.info('updateSelectiveSync ended without changes', {category: 'sync.selective', err});
        if(err) return callback(err);
      ...
      return updateChildren(rootNode.children, err => {
        logger.info('updateSelectiveSync ended', {category: 'sync.selective', err});
        if(err) return callback(err);
      ...
      function ignoreNode(node, ignoreCb) {
        async.parallel([
          ...
          (cb) => {
            logger.debug('removeing ignored node from fs', {category: 'sync.selective', node, instanceDirPath});
      ...
      function unignoreNode(node, unignoreCb) {
        async.parallel([
          (cb) => {
            logger.debug('addign unignored node to syncDb', {category: 'sync.selective', node, instanceDirPath});
  ...
  /**
   * Updates remote paths and ignored shares
   *
   * @param {Function} callback - callback function
   * @returns {void}
   */
  updateIgnoreDb: function(callback) {
    async.parallel([
      this.updateRemotePaths,
      this.ignoreNewShares
    ], callback);
  },
   ...
   * @returns {void}
   */
  updateRemotePaths: function(callback) {
  ...
  /**
   * Ignores all new shares by default
   *
   * @param {Function} callback - callback function
   * @returns {void}
   */
  ignoreNewShares: function(callback) {
    logger.info('Ignore new shares', {category: 'sync.selective'});

    async.waterfall([
      (cb) => {
        async.parallel([
          (cbParallel) => {
            knownSharesDb.find({}, cbParallel);
          },
          (cbParallel) => {
            blnApi.queryNodes({shared:true, reference:{'$exists':true}}, cbParallel);
          }
        ], cb);
      },
      (results, cb) => {
        const [knownShares, nodes] = results;

        async.map(nodes, (node, cbMap) => {
          if(knownShares.find(share => (share.remoteId === node.id))) {
            cbMap(null);
          } else {
            async.parallel([
              (cbParallel) => {
                syncDb.findByRemoteId(node.id, (err, syncedNode) => {
                  ...
                  if(syncedNode) return cbParallel(null);

                  logger.info('ignoring new share', {category: 'sync.selective', node});

                  ignoreDb.add({remoteId: node.id, remotePath: node.path, path: node.path}, cbParallel);
                });
              },
              (cbParallel) => {
                knownSharesDb.insert({remoteId: node.id}, cbParallel);
              }
            ], cbMap);
          }
        }, cb);
      ...
      if(err) {
        logger.error('Could not ignore new shares', {category: 'sync.selective', err});
        return callback(err);
      }
      ...
      fsWrap.readdir(pathCollection, (err, nodes) => {
        ...
        async.each(nodes, (name, cb) => {
          ...
          try {
            ...
            logger.warning('Got lstat error', {category: 'sync.selective', nodePath, code: e.code});
            return cb(e);
          }
