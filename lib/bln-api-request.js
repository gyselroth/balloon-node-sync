...
function getAuthorizationHeader() {
  if(['oidc', 'token'].includes(config.get('authMethod'))) {
    return 'Bearer ' + config.get('accessToken');
  } else if(config.get('authMethod') === 'basic' && config.get('username')){
    return 'Basic ' + (Buffer.from(config.get('username') + ':' + config.get('password')).toString('base64'));
  } else {
...
function handleResponse(response, reqOptions, body, callback) {
  ...
  if(response.statusCode === 403 && (body && body.error === 'Balloon\\App\\Idp\\Exception\\MultiFactorAuthenticationRequired')) {
    logger.warning('API: Got Status Code 403 MFA required');

    return callback(new BlnApiRequestError('MFA required', 'E_BLN_API_REQUEST_MFA_REQUIRED'));
  }
    ...
    if(body && (body.error || body.message || body.code)) {
      ...
      if(body.code) {
        switch(body.code) {
          ...
          break;
          case 20:
            errCode = 'E_BLN_API_REQUEST_SHARE_CANT_BE_CHILD_OF_SHARE';
          break;
          case 25:
          ...
          break;
          case 65:
            errCode = 'E_BLN_API_REQUEST_QUOTA_FULL';
          break
        }
      }
    }
    ...
    return request(reqOptions, (err, response, body) => {
      if(err) {
        if(err.code && ['ENOTFOUND', 'ETIMEDOUT', 'ENETUNREACH', 'EHOSTUNREACH', 'ECONNREFUSED', 'EHOSTDOWN', 'ESOCKETTIMEDOUT', 'ECONNRESET'].includes(err.code)) {
          err = new BlnApiRequestError(`${err.message} (${err.code})`, 'E_BLN_API_REQUEST_NETWORK');
        }
  ...
  sendStreamingRequest: function(method, endpoint, params) {
    ...
    req.on('response', function(response) {
      handleResponse(response, reqOptions, function(err) {
        if(err) {
          req.emit('bln-error', err, response);
        } else {
