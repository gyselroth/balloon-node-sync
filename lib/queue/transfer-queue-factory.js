        ...
        var progress = blnApi.downloadFile(node.remoteActions.create.remoteId, node.remoteActions.create.version, node, (err) => {
          if(err) {
            switch(err.code) {
              ...
              break;
              default:
                var errorMessage = `file download for node with id '${node.remoteActions.create.remoteId}' failed`;
                logger.error(errorMessage, {category: 'transfer-queue', message: err.message, code: err.code});
        ...
        var progress = blnApi.uploadFile(node, (err, result) => {
          if(err) {
            switch(err.code) {
              ...
              break;
              default:
                var errorMessage = `Upload of '${utility.joinPath(node.parent, node.name)}' failed`;
                logger.error(errorMessage, {category: 'transfer-queue', message: err.message, code: err.code});
                emitStateEvent(task, 'error', err);
          ...
          blnApi.getAttributes(node, ['hash', 'version', 'id'], (err, attributes) => {
            if(err) {
              switch(err.code) {
                ...
                break;
                default:
                  logger.warning('getAttributes failed: unauthorized', {category: 'transfer-queue', message: err.message, code: err.code});
                  return done(null);
                break;
              }
