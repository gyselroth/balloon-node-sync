...
var remoteDeltaLogDb = require('./delta/remote-delta-log-db.js');
...
function cleanremoteDeltaLogDb(callback) {
  if(remoteDeltaLogDb.isConnected() === false) return (callback(null));

  var compareDate = getCompareDate(72 * 60 * 60 * 1000);

  remoteDeltaLogDb.removeCreatedBefore(compareDate, (err) => {
