...
const BlnWatcherError = require('../../errors/bln-watcher.js');
...
RemoteWatcherFactory.prototype.start = function() {
  ...
  this.pollIntervall = setInterval(() => {
    blnApi.nodeDelta({limit: 1, cursor: lastCursor.read()}, (err, data) => {
      if(err) {
        var errToEmit = new BlnWatcherError('Recievieng node delta had error', 'E_BLN_REMOTE_WATCHER_DELTA');
        errToEmit.origErr = err;
        this.emit('error', errToEmit);
        return;
      }
