...
var localDelta = {
    ...
    fsWrap.readdir(dirPath, (err, nodes) => {
      ...
      async.eachSeries(nodes, (node, cb) => {
        ...
        try {
          ...
          logger.error('LOCAL DELTA: findChanges: got lstat error on node', {nodePath, code: e.code});
          throw e;
        }
   ...
   * @returns {void} - no return value
   */
  analyzeNodeForChanges: function(name, dirPath, oldDirPath, parentNodeId, stat, callback) {
        ...
        async.eachSeries(nodes, (node, cb) => {
          ...
          process.nextTick(() => {
            if(fsWrap.existsSync(nodePath) === false) {
              ...
              deleteNode(node, false, done);
            } else {
              try {
                var stat = fsWrap.lstatSync(nodePath);
              } catch(e) {
                logger.error('LOCAL DELTA: findDeletedNodes: got lstat error on node', {nodePath, code: e.code});
                throw e;
              }
