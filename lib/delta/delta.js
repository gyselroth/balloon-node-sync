  ...
  async.parallel([
    (cb) => {
      ...
      syncDb.processChildren(destNode._id, (childNode, processCb) => {
        ...
        if(destRemoteId) {
          ...
          childNode.localActions.move = {oldParent: childNode.parent, actionInitialized: new Date()};
    ...
    (cb) => {
      ...
      if(destNode.localActions && destNode.localActions.create) {
        ...
        srcNode.localActions = {delete: {actionInitialized: new Date()}};
      }

      syncDb.update(srcNode._id, srcNode, cb);
    },
    (cb) => {
