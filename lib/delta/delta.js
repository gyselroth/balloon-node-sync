...
function resolveFileConflicts(node, callback) {
  ...
  if(rActions.create && lActions.create) {
    ...
    try {
      ...
      logger.error('resolveFileConflicts failed', {category: 'sync-delta', node, err: e});
    ...
    if(fsWrap.existsSync(targetPath)) {
      ...
      return syncDb.findByIno(targetStat.ino, (err, syncedNode) => {
        if(err) {
          ...
          throw (new BlnDeltaError('Database error'));
        }
        ...
        logger.debug('Renaming conflict file', {category: 'sync-delta', parent, name, syncedNode});
...
function renameRemoteNode(node, name, callback) {
  ...
  if(!rActions.create && fsWrap.existsSync(oldLocalPath)) {
    try {
      ...
      logger.error('rename local node failed', {category: 'sync-delta', oldLocalPath, newLocalPath, err})
  ...
  return blnApi.renameNode({remoteId, name: node.name}, (err) => {
    if(err) {
      logger.error('rename remote node failed', {category: 'sync-delta', params: {remoteId, name: node.name}, err})
      throw new BlnDeltaError(err.message);
