  ...
  if(rActions && (rActions.create || rActions.rename || rActions.move)) {
    ...
    if(fsWrap.existsSync(targetPath)) {
      ...
      return syncDb.findByIno(targetStat.ino, (err, syncedNode) => {
        if(err) {
          logger.warning('database error', {category: 'sync-delta', err});
          throw (new BlnDeltaError('Database error'));
        }

        if(!syncedNode) {
          logger.warning('node not found in database even if it should exists:', {category: 'sync-delta', targetPath, node, targetStat});
        ...
        logger.debug('Renaming conflict file', {category: 'sync-delta', parent, name, targetStat, node, syncedNode});
