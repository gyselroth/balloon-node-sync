 ...
 * eg: directory A has been moved remotely to same path as directory B has been moved localy
 *
 * @param {Object} srcNode - remote node from database
 * @param {Object} destNode - local node from database
 * @param {Function} callback - callback function
 * @returns {void} - no return value
 */
function mergeDirectories(srcNode, destNode, callback) {
  ...
  if(destLocalActions && destLocalActions.rename) {
    destParentName = destLocalActions.rename.oldName;
  }

  if(destLocalActions && destLocalActions.move) {
 ...
 * @returns {void} - no return value
 */
function renameConflictFile(newParent, oldPath, name, node, conflictingNode, callback) {
  var newLocalName = utility.renameConflictFile(newParent, name);
  ...
  if(!conflictingNode.localActions.create) {
    //if the node was just localy created and not localy renamed, no need to rename it remotely
