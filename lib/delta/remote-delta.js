  ...
  blnApi.nodeDelta(params, (err, data) => {
    ...
    async.parallel([
      (cb) => {
        ...
        process.nextTick(() => {
          Object.assign(params, {cursor: data.cursor});
          fetchDelta(params, cb);
        });
      },
      (cb) => {
        async.series([
          (cbSeries) => {
            async.each(data.nodes, (node, cbEach) => {
              // if params.id is set, this is a forced download of a collection,
              // hence do not ignore the node (eg. after unignoring a share)
              if(node.shared === true && node.reference === true && !params.id) {
                logger.info('ignoring new share', {category: 'sync-remote-delta', node});
                ignoreDb.add({remoteId: node.id, remotePath: node.path, path: node.path}, (err) => {
                  if(err) {
                    logger.warning('Could not ignore new share', {category: 'sync-remote-delta', err});
                    return cbEach(err);
                  }
          ...
          (cbSeries) => {
            groupDelta(data.nodes, cbSeries);
          }
        ], cb);
      }
    ], (err, results) => {
